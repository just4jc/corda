...
import net.corda.node.services.messaging.NodeMessagingClient.Companion.RPC_NOTIFICATIONS_QUEUE
...
abstract class MQSecurityTest : NodeBasedTest() {
    ...
    @Test
    fun `consume message from RPC notifications queue`() {
        assertConsumeAttackFails(RPC_NOTIFICATIONS_QUEUE)
    }

    @Test
    fun `send message to RPC notifications address`() {
        assertSendAttackFails(RPC_NOTIFICATIONS_QUEUE)
    }
