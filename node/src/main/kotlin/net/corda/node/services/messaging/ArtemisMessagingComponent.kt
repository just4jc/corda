...
abstract class ArtemisMessagingComponent() : SingletonSerializeAsToken() {
    ...
    companion object {
        ...
        // System users must contain an invalid RPC username character to prevent any chance of name clash
        const val NODE_USER = "SystemUsers/Node"
        const val PEER_USER = "SystemUsers/Peer"

        const val P2P_PREFIX = "p2p."
        const val PEERS_PREFIX = "${P2P_PREFIX}peers."
        const val CLIENTS_PREFIX = "clients."
        const val P2P_QUEUE = "${P2P_PREFIX}inbound"
        const val RPC_REQUESTS_QUEUE = "rpc.requests"

        @JvmStatic
        val NETWORK_MAP_ADDRESS = SimpleString("${P2P_PREFIX}networkmap")
