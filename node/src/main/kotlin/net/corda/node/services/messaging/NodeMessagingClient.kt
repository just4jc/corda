...
class NodeMessagingClient(override val config: NodeConfiguration,
    ...
    companion object {
        ...
        const val RPC_NOTIFICATIONS_QUEUE = "activemq.notifications"
    ...
    fun start(rpcOps: RPCOps, userService: RPCUserService) {
        state.locked {
            ...
            session.createTemporaryQueue(RPC_NOTIFICATIONS_QUEUE, RPC_NOTIFICATIONS_QUEUE, "_AMQ_NotifType = 1")
            rpcConsumer = session.createConsumer(RPC_REQUESTS_QUEUE)
            rpcNotificationConsumer = session.createConsumer(RPC_NOTIFICATIONS_QUEUE)
