...
class ArtemisMessagingServer(override val config: NodeConfiguration,
    ...
    private fun onNetworkChange(change: NetworkMapCache.MapChange) {
        val address = change.node.address as? ArtemisAddress ?: return
        ...
        fun deployNewBridge() {
            val query = activeMQServer.queueQuery(address.queueName)
            if (query.isExists) {
    ...
    private fun parseKeyFromQueueName(name: String): CompositeKey {
    ...
    private fun createArtemisConfig(): Configuration = ConfigurationImpl().apply {
        ...
        // Lockdown the cluster user as it has super-user access
     ...
     * 1. The node hosting us and any of its logically connected components. These are given full access.
    ...
    private fun ConfigurationImpl.configureAddressSecurity() {
        val nodeFullAccess = Role(NODE_ROLE, true, true, true, true, true, true, true, true)
        securityRoles["#"] = setOf(nodeFullAccess)  // Do not add any other roles to "#" as it applies to all addresses (except those specified below)
        securityRoles[P2P_QUEUE] = setOf(nodeFullAccess, restrictedRole(PEER_ROLE, send = true))
        securityRoles[RPC_REQUESTS_QUEUE] = setOf(nodeFullAccess, restrictedRole(RPC_ROLE, send = true))
     ...
     * is the same as our one in our key store. Then we're given full access to the broker. If they connect with
            ...
            val validatedUser = if (username == PEER_USER || username == NODE_USER) {
                ...
                val role = if (username == NODE_USER) {
                    if (peerCertificate.publicKey != ourPublicKey) {
                        throw FailedLoginException("Only the node can login as Node")
                    }
                    NODE_ROLE
                } else {
